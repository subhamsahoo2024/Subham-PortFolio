...
export default function ProjectsGrid({ theme }: ProjectsGridProps) {
  ...
  const projectImageMap: Record<string, string> = {
    guardianlink: "/images/Guardian-link.png",
    navx: "/images/navx.png",
    "smart-heart": "/images/smartheart.png",
    "intervion-ai": "/images/intervionAI.png",
    innovex: "/images/Innovex.png",
    blocktracked: "/images/blocktracked.png",
    "think-book": "/images/thinkbook.png",
    "mini-games-hub": "/images/mini-game-hub.png",
  };

  const categories = ["All", "AI", "Full-Stack", "Web3"];
  ...
  const cardVariants = {
    ...
    hover: {
      ...
      boxShadow:
        theme === "dark"
          ? "0 20px 40px -15px rgba(99, 102, 241, 0.35)"
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-1/4 right-0 w-[500px] h-[500px] rounded-full blur-[120px] -z-10 transition-colors ${
          theme === "dark" ? "bg-emerald-500/5" : "bg-emerald-500/3"
        }`}
      ></div>
      <div
        className={`absolute bottom-1/4 left-0 w-[400px] h-[400px] rounded-full blur-[100px] -z-10 transition-colors ${
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <div
            className={`flex items-center gap-1.5 p-1 rounded-xl border backdrop-blur-md w-fit transition-colors ${
              theme === "dark"
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              ...
              >
                ...
                <div
                  className="absolute inset-0 bg-[linear-gradient(to_right,var(--inner-grid)_1px,transparent_1px),linear-gradient(to_bottom,var(--inner-grid)_1px,transparent_1px)] bg-[size:1.5rem_1.5rem] rounded-2xl pointer-events-none opacity-40"
                  style={
                    {
                      "--inner-grid":
                        theme === "dark" ? "#312e810a" : "#6366f108",
                ...
                <div className="relative z-10">
                  <div
                    className={`relative w-full h-44 mb-6 rounded-xl border overflow-hidden transition-all duration-300 group-hover:border-indigo-500/30 ${
                      theme === "dark"
                  ...
                  >
                    ...
                    <div className="absolute top-3 left-3 flex items-center gap-1.5 rounded-full bg-slate-950/70 px-2.5 py-1 text-[9px] font-mono text-slate-200 backdrop-blur-md">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                      {project.demo ? "Live preview" : "Preview image"}
                    </div>
                  ...
                  {/* Title & Description */}
                  <h3
                    className={`font-display font-bold text-xl sm:text-2xl mb-3 transition-colors ${
                      theme === "dark"
                  ...
                  <p
                    className={`font-light text-sm leading-relaxed mb-6 transition-colors ${
                  ...
                  <div
                    className={`mb-6 pt-4 border-t ${theme === "dark" ? "border-white/5" : "border-slate-100"}`}
                  >
                    <span className="text-[10px] font-mono uppercase text-slate-500 tracking-wider block mb-2.5">
                    ...
                    <ul className="space-y-2">
                      {project.features.map((feature, fIdx) => (
                        <li
                          key={fIdx}
                          className={`flex items-start gap-2.5 text-xs leading-normal font-light transition-colors ${
                            theme === "dark"
                              ? "text-slate-300"
                              : "text-slate-700"
                          }`}
                ...
                <div className="relative z-10">
                  ...
                  {/* CTA Action Buttons */}
                  <div
                    className={`flex items-center gap-3 pt-4 border-t ${theme === "dark" ? "border-white/5" : "border-slate-100"}`}
                  >
                    <a
                      href={project.github}
                      target="_blank"
